(()=>{
  ...
  frameCrop=function(f){if(f&&f._splitCanvas)return f._splitCanvas;return originalFrameCrop(f)};
  const originalUpdateActionButtons=updateActionButtons;
  updateActionButtons=function(){originalUpdateActionButtons();splitBtn.disabled=selected<0||selected>=frames.length};

  function cloneCanvas(c){const out=document.createElement('canvas');out.width=c.width;out.height=c.height;const ctx=out.getContext('2d');ctx.imageSmoothingEnabled=false;ctx.drawImage(c,0,0);return out}
  function openEditor(){if(selected<0||selected>=frames.length)return;stopAnim?.();editingIndex=selected;baseFrame=frames[selected];source=cloneCanvas(frameCrop(baseFrame));points=[];lastSplit=null;root.classList.add('open');document.body.style.overflow='hidden';applyBtn.disabled=true;drawWork();drawPreviews(null)}
  function closeEditor(){root.classList.remove('open');document.body.style.overflow='';drawing=false;points=[];lastSplit=null}
  splitBtn.addEventListener('click',openEditor);cancelBtn.addEventListener('click',closeEditor);
  clearBtn.addEventListener('click',()=>{points=[];lastSplit=null;applyBtn.disabled=true;drawWork();drawPreviews(null)});
  zoom.addEventListener('input',drawWork);dir.addEventListener('change',()=>{if(points.length>1)refreshSplit();else drawWork()});overlap.addEventListener('input',()=>{if(points.length>1)refreshSplit()});
  ...
  function buildBoundary(axisLength,raw,keyMain,keyCross){
    const pts=raw.map(p=>({m:p[keyMain],c:p[keyCross]})).sort((a,b)=>a.m-b.m);const compact=[];
    for(const p of pts){const last=compact[compact.length-1];if(last&&Math.abs(last.m-p.m)<.4){last.c=(last.c+p.c)/2;last.m=(last.m+p.m)/2}else compact.push({...p})}
    if(compact.length<2)return null;const out=new Float32Array(axisLength);let seg=0;
    for(let i=0;i<axisLength;i++){while(seg<compact.length-2&&i>compact[seg+1].m)seg++;if(i<=compact[0].m)out[i]=compact[0].c;else if(i>=compact[compact.length-1].m)out[i]=compact[compact.length-1].c;else{const a=compact[seg],b=compact[seg+1],t=(i-a.m)/Math.max(.0001,b.m-a.m);out[i]=a.c+(b.c-a.c)*t}}
    return out;
  }
  function splitCanvas(){
    if(!source||points.length<2)return null;const sw=source.width,sh=source.height,ov=Math.max(0,+overlap.value||0),mode=dir.value;const srcCtx=source.getContext('2d',{willReadFrequently:true}),src=srcCtx.getImageData(0,0,sw,sh),aData=new ImageData(sw,sh),bData=new ImageData(sw,sh);const line=edgeExtendedPoints();
    const boundary=mode==='tb'?buildBoundary(sw,line,'x','y'):buildBoundary(sh,line,'y','x');if(!boundary)return null;
    for(let y=0;y<sh;y++)for(let x=0;x<sw;x++){const si=(y*sw+x)*4,alpha=src.data[si+3];if(!alpha)continue;const d=mode==='tb'?y-boundary[x]:x-boundary[y];if(d<=ov){aData.data[si]=src.data[si];aData.data[si+1]=src.data[si+1];aData.data[si+2]=src.data[si+2];aData.data[si+3]=alpha}if(d>=-ov){bData.data[si]=src.data[si];bData.data[si+1]=src.data[si+1];bData.data[si+2]=src.data[si+2];bData.data[si+3]=alpha}}
  ...
  function alphaBounds(c){const ctx=c.getContext('2d',{willReadFrequently:true}),d=ctx.getImageData(0,0,c.width,c.height).data;let minx=c.width,miny=c.height,maxx=-1,maxy=-1;for(let y=0;y<c.height;y++)for(let x=0;x<c.width;x++)if(d[(y*c.width+x)*4+3]){if(x<minx)minx=x;if(x>maxx)maxx=x;if(y<miny)miny=y;if(y>maxy)maxy=y}return maxx<0?null:{x:minx,y:miny,w:maxx-minx+1,h:maxy-miny+1}}
  ...
  applyBtn.addEventListener('click',()=>{const result=lastSplit||splitCanvas();if(!result||editingIndex<0||editingIndex>=frames.length)return;const ba=alphaBounds(result.a),bb=alphaBounds(result.b);if(!ba||!bb){setStatus('分割結果の片方が空です。分割線を調整してください。');return}const old=frames[editingIndex],ca=cropMasked(result.a,ba),cb=cropMasked(result.b,bb);const fa={x:old.x+ba.x,y:old.y+ba.y,w:ba.w,h:ba.h,_splitCanvas:ca};const fb={x:old.x+bb.x,y:old.y+bb.y,w:bb.w,h:bb.h,_splitCanvas:cb};let slot=-1;if(typeof layoutSlots!=='undefined'&&Array.isArray(layoutSlots))slot=layoutSlots.indexOf(old);frames.splice(editingIndex,1,fa,fb);if(slot>=0)layoutSlots.splice(slot,1,fa,fb);selected=editingIndex;mergeTargets=[];closeEditor();renderSource();renderFrames();updateActionButtons();drawPreview(selected);setStatus(`フレーム #${editingIndex+1} を A/B の2フレームへ分割しました。補完幅 ${Math.max(0,+overlap.value||0)}px。`)});

  document.addEventListener('keydown',e=>{if(!root.classList.contains('open'))return;if(e.key==='Escape')closeEditor()});updateActionButtons();
})();
